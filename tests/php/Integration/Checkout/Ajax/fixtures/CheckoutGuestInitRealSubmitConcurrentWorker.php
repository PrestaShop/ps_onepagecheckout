...
if ($argc !== 6 && $argc !== 7) {
    fwrite(STDERR, "Usage: php CheckoutGuestInitRealSubmitConcurrentWorker.php <cartId> <email> <token> <barrierId> <workerId> [workersCount]\n");
    exit(2);
}
...
final class ConcurrentRealSubmitTranslator implements TranslatorInterface
{
...
final class ConcurrentRealSubmitEligibleCart extends Cart
{
...
final class ConcurrentRealSubmitCheckoutForm extends OnePageCheckoutForm
{
...
final class ConcurrentRealSubmitCheckoutGuestInitHandler extends OnePageCheckoutGuestInitHandler
{
 ...
 */
function buildGuestInitRequestParams(ConcurrentRealSubmitCheckoutForm $form, string $email, string $token): array
{
    $params = [
        'email' => $email,
        'token' => $token,
...
try {
    ...
    $contextCart = new ConcurrentRealSubmitEligibleCart();
    ...
    $translator = new ConcurrentRealSubmitTranslator();
    ...
    $opcForm = new ConcurrentRealSubmitCheckoutForm(
        $context->smarty,
        $context,
        $context->language,
        $translator,
    ...
    $handler = new ConcurrentRealSubmitCheckoutGuestInitHandler(
        $context,
        $opcForm,
        $translator,
        $customerPersister,
    ...
    $requestParameters = buildGuestInitRequestParams($opcForm, $email, $expectedToken);
