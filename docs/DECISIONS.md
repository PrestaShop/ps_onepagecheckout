...
### D-025
- Context: the client-side request queue (D-019 era, PR #89) serializes ALL OPC ajax because every endpoint is a partial view over a single shared resource — the cart — mutated through whole-row ObjectModel saves (read-modify-write, no locking), historically amplified by the temp-address mount/restore double-write around each inline computation. The queue therefore carries two roles at once: correctness (intent ordering, response coherence between partial views) and hygiene (burst coalescing). A client-side mutex cannot protect against out-of-band writers (second tab, back office, payment-module server calls), and the specs (`opc.e2e.tests` ffb5911) now codify the queue's observable semantics.
- Decision: staged target architecture, each step enabling the next. (1) Replace whole-row cart saves with targeted atomic column updates and make the temp-address cleanup restore compare-and-set — correctness moves server-side, covering out-of-band writers. (2) Consolidate the partial endpoints into a checkout-state endpoint returning ONE consistent snapshot (carriers, payment methods, totals, context_refresh) per buyer intent — response coherence then comes from the shape of responses instead of the order of requests. (3) The client layer shrinks to a generation guard (latest snapshot wins), a single in-flight mutation, and an intent debounce. (4) EVENT CONTRACT PRESERVED: a single snapshot-applier derives and emits the standard theme/OPC events (`updatedDeliveryForm`, `updatedCart`, `opcCarriersUpdated`, `opcPaymentMethodsUpdated`, ...) from the applied state diff — after `context_refresh` (D-024) and with the binary re-arm (D-023) — so unmodified modules (payment SDK reloads included) keep working; accepted delta: one coherent event burst per intent instead of per HTTP request, same class as the documented `actionCarrierProcess` render-tick delta (D-020).
- Impact: the queue is demoted from correctness guarantee to trivial hygiene; write races are closed even against out-of-band writers; per-endpoint queue granularity is explicitly rejected (all OPC endpoints contend on the same cart — the global lane IS the per-resource lane); the module-facing event contract is unchanged by design rather than by accident.
